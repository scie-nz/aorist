    ...
    Self::PreambleType: Preamble,
{
    ...
    fn get_block_comment(&self) -> AString {
        match self.get_constraint_title() {
            AOption(ROption::RSome(t)) => match self.get_constraint_body() {
                AOption(ROption::RSome(b)) => format!(
                    "## {}\n{}",
                    t,
                    b.as_str()
                        .to_string()
                ...
                .as_str()
                .into(),
                AOption(ROption::RNone) => format!("## {}", t).as_str().into(),
            },
            AOption(ROption::RNone) => format!("## {}", self.get_constraint_name()).as_str().into(),
        }
    }
                    ...
                    for (_task_key, task_val) in dict.elems() {
                        if let AST::Dict(param_dict_rw) = task_val {
                            ...
                            for (key, val) in param_dict.elems() {
                                if let AOption(ROption::RSome(_ancestors)) = val.get_ancestors() {
